...
local function xp_parse_time(t)
    ...
    if p[3] == nil then
        ...
        p = { t:match("^%s*(%d+)([/-])(%a+)(.*)") }
        order = DMY
    end                                                                                        -- number-word (4-Jul)
    if p[3] == nil then
        D("match 3")
        p = { t:match("^%s*(%a+)([/-])(%d+)(.*)") }
        order = MDY
    end                                                                                        -- word-number (Jul-4)
    if p[3] ~= nil then
...
local function xp_split(argv)
    ...
    local rest = string.gsub(str or "", "([^" .. sep .. "]*)" .. sep, function(m)
        table.insert(arr, m)
        return ""
    end)
...
local nativeFuncs = {
    ...
    ,
    ['round'] = { nargs = 1, impl = function(argv)
        local n = argv[1]
        local p = argv[2] or 0
    ...
    ,
    ['randomseed'] = { nargs = 0, impl = function(argv)
        local s = argv[1] or os.time()
        math.randomseed(s)
        return s
    end }
    ,
    ...
    ,
    ['len'] = { nargs = 1, impl = function(argv) if isNull(argv[1]) then return 0 elseif type(argv[1]) == "table" then return
            xp_tlen(argv[1]) else return string.len(tostring(argv[1])) end end }
    ,
    ['sub'] = { nargs = 2, impl = function(argv)
        local st = tostring(argv[1])
        local p = argv[2]
        local l = (argv[3] or -1)
        return string.sub(st, p, l)
    end }
    ,
    ['find'] = { nargs = 2, impl = function(argv)
        local st = tostring(argv[1])
        local p = tostring(argv[2])
        local i = argv[3] or 1
        return (string.find(st, p, i) or 0)
    end }
    ,
    ...
    ,
    ['tonumber'] = { nargs = 1, impl = function(argv)
        if base.type(argv[1]) == "boolean" then if argv[1] then return 1 else return 0 end end
        return tonumber(argv[1], argv[2] or 10) or evalerror('Argument could not be converted to number')
    end }
    ,
    ...
    ,
    ['choose'] = { nargs = 2, impl = function(argv)
        local ix = argv[1]
        if ix < 1 or ix > (#argv - 2) then return argv[2] else return argv[ix + 2] end
    end }
    ,
    ...
    ,
    ['list'] = { nargs = 0, impl = function(argv)
        local b = deepcopy(argv)
        b.__context = nil
        return b
    end }
    ,
    ['first'] = { nargs = 1, impl = function(argv)
        local arr = argv[1]
        if base.type(arr) ~= "table" or #arr == 0 then return NULLATOM else return arr[1] end
    end }
    ,
    ['last'] = { nargs = 1, impl = function(argv)
        local arr = argv[1]
        if base.type(arr) ~= "table" or #arr == 0 then return NULLATOM else return arr[#arr] end
    end }
}
...
local function scan_numeric(expr, index)
    ...
    if (ch == 'e' or ch == 'E') and radix == 10 then
        ...
        while (index <= len) do
            ch = string.sub(expr, index, index)
            if neg == nil and ch == "-" then
                neg = true
            elseif neg == nil and ch == "+" then
                neg = false
            else
                i = string.byte(ch) - 48
                if i < 0 or i > 9 then break end
...
local function coerce(val, typ)
    ...
    if typ == "boolean" then
        ...
        elseif vt == "string" then
            if string.lower(val) == "true" or val == "yes" or val == "1" then
                return true
            elseif string.lower(val) == "false" or val == "no" or val == "0" then
                return false
            else
                return #val ~= 0             -- empty string is false, all else is true
            end
    ...
    elseif typ == "string" then
        ...
        elseif isNull(val) then
            return ""                     -- null coerces to empty string within expressions
        end
...
_run = function(atom, ctx, stack)
    if not isAtom(atom) then
        D("Invalid atom: %1", atom)
        evalerror("Invalid atom")
    end
    stack = stack or {}
    local v = nil
    local e = atom
    ...
    elseif isAtom(e, BINOP) then
        ...
        elseif e.op == '<' then
            if not check_operand(v1, { "number", "string" }, v2) then
                evalerror("Invalid comparison ("
                    .. base.type(v1) .. e.op .. base.type(v2) .. ")", e.pos)
            end
            v = v1 < v2
        elseif e.op == '<=' then
            if not check_operand(v1, { "number", "string" }, v2) then
        ...
        elseif e.op == '=' then
            ...
            if v1.index ~= nil then
                ...
                if type(ctx.__lvars[v1.name]) ~= "table" then evalerror("Target is not an array (" .. v1.name .. ")",
                        e.pos) end
    ...
    elseif isAtom(e, FREF) then
        ...
        elseif e.name == "iterate" then
            ...
            if v1 ~= nil and not isNull(v1) then
                ...
                local iexp = isAtom(e.args[2], CONST) and _comp(e.args[2].value, ctx) or
                e.args[2]                                                                              -- handle string as expression
        ...
        elseif e.name == "map" then
            ...
            if v1 ~= nil and not isNull(v1) then
                ...
                local iexp
                if #e.args > 1 then
                    iexp = isAtom(e.args[2], CONST) and _comp(e.args[2].value, ctx) or
                    e.args[2]                                                                        -- handle string as expression
                    -- if not isAtom( iexp ) then evalerror("iterate() arg 2 must be expression or string containing expression", e.pos) end
                else
            ...
            if nativeFuncs[e.name] then
                ...
                if (argc < nativeFuncs[e.name].nargs) then evalerror(
                    "Insufficient arguments to " ..
                    e.name .. "(), need " .. nativeFuncs[e.name].nargs .. ", got " .. argc, e.pos) end
            end
