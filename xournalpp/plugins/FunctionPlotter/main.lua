...
function showDialog()
    ...
    if not hasLgi then
        app.openDialog("You should have the Lua lgi-module installed and included in your Lua package path.", { "OK" },
            "", true)
        return
    end
    ...
    function ui.btnAddGraph.on_clicked()
        ...
        if drawGrid then
            ...
            for i = (wc.xMin // wc.xUnit) + 1, (wc.xMax // wc.xUnit) do
                ...
                table.insert(strokes,
                    { x = { xval, xval }, y = { my, My }, width = lwGrid, lineStyle = gridStyle, color = colGrid })
            end
            -- draw horizontal grid lines
            for i = (wc.yMin // wc.yUnit) + 1, (wc.yMax // wc.yUnit) do
                local yval = fitY(i * wc.yUnit)
                table.insert(strokes,
                    { x = { mx, Mx }, y = { yval, yval }, width = lwGrid, lineStyle = gridStyle, color = colGrid })
            end
        end
        ...
        if displayNumbers then
            ...
            for i = (wc.yMin // wc.yUnit) + 1, (wc.yMax // wc.yUnit) do
                ...
                if (v ~= 0) then
                    table.insert(texts,
                        { text = text, font = font, x = x0 + tickHeight, y = yval - font.size, color = colAxis })
                end
            end
        end
    ...
    function packRgba(rgba)
        return math.floor(256 ^ 2 * 255 * rgba.red + 256 * 255 * rgba.green + 255 * rgba.blue),
            math.floor(rgba.alpha * 255)
    end
